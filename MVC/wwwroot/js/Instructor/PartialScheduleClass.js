document.addEventListener("DOMContentLoaded", function () {
  ...
  const equipmentByClassType = {
    ...
    "Strength Training": [
      ...
      },
      {
        value: "Weighted Vest",
        icon: "🎽",
        description: "Adds resistance to bodyweight exercises",
      },
    ],
  };
  ...
  function validateEquipment() {
    ...
    if ($feedbackElement.length === 0) {
      $equipmentDropdown.append(
        '<div class="invalid-feedback">Please select at least one equipment item</div>'
      );
    }
    ...
    if (!classType || !equipmentByClassType[classType]) {
      ...
      $equipmentDropdown
        .find(".invalid-feedback")
        .text("Please select a valid class type first")
        .show();
      return false;
    }
    ...
    if (!validEquipment) {
      ...
      $equipmentDropdown
        .find(".invalid-feedback")
        .text("Some selected equipment is not valid for this class type")
        .show();
      return false;
    }
  ...
  $(document).on("click", function (e) {
    if (!$(e.target).closest(".equipment-dropdown").length) {
      $(".equipment-dropdown .dropdown-menu").removeClass("show");
    }
  });

  // Toggle dropdown on button click
  $("#equipmentDropdown").on("click", function (e) {
    e.stopPropagation();
    $(".equipment-dropdown .dropdown-menu").toggleClass("show");
  });

  // Prevent dropdown from closing when clicking inside
  $(".equipment-dropdown .dropdown-menu").on("click", function (e) {
    e.stopPropagation();
  });
  ...
  async function fetchAddress(lat, lng) {
    try {
      ...
      const englishAddress =
        data.display_name ||
        `Near ${data.address.road || data.address.city || "Selected Location"}`;
    ...
    async function processFilesInBatch(files) {
      ...
      for (const file of files) {
        try {
          ...
          const isBannerSize =
            dimensions.width === BANNER_DIMENSIONS.width &&
        ...
        if (bannerFile.size <= BANNER_MAX_SIZE) {
          ...
          if (bannerSizedFiles.length > 1) {
            Swal.fire({
              ...
              text: `Only using the first banner image. ${
                bannerSizedFiles.length - 1
              } additional banner images were not added.`,
              icon: "error",
              confirmButtonText: "OK",
            });
          }
      ...
      if (normalImagesNeeded > 0 && normalSizedFiles.length > 0) {
        ...
        if (rejectedForSize > 0) {
          Swal.fire({
            title: "Invalid Images",
            text: `${rejectedForSize} normal image(s) exceeded the maximum size of ${
              NORMAL_MAX_SIZE / 1024 / 1024
            }MB`,
            icon: "error",
            confirmButtonText: "OK",
          });
        ...
        if (normalSizedFiles.length > normalImagesNeeded) {
          Swal.fire({
            title: "Invalid Images",
            text: `Only using ${normalImagesNeeded} normal images. ${
              normalSizedFiles.length - normalImagesNeeded
            } additional normal images were not added.`,
            icon: "error",
            confirmButtonText: "OK",
          });
        }
  ...
  function validateDateRange() {
    ...
    if (daysDifference < 2) {
      $endDate
        .addClass("is-invalid")
        .next(".invalid-feedback")
        .text("Minimum date range is 2 days");
      return false;
    }
            ...
            html: `
					  <div class="prediction-result">
						  ...
						  <p>
							  ...
							  <span class="text-${data.is_popular ? "success" : "danger"}">
								  ${data.is_popular ? "Likely to be Popular" : "May Need Promotion"}
							  </span>
						  </p>
						  <p>
							  <strong>Confidence Level:</strong> 
							  <span class="text-${confidenceColor}">${data.confidence_level}</span>
						  </p>
						  <p>
