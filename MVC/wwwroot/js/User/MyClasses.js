...
$("document").ready(function () {
    if (typeof loadUserWalletBalance === "function") {
        loadUserWalletBalance();
    } else {
        // Optionally, try again after a short delay if not yet loaded
        setTimeout(function() {
            if (typeof loadUserWalletBalance === "function") {
                loadUserWalletBalance();
            }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.success && response.data) {
                ...
                // Optionally show a message in .class-grid
                document.querySelector('.class-grid').innerHTML = `<div class="no-classes-message">No classes found.</div>`;
            }
        },
        error: function (xhr, status, error) {
            ...
            // Optionally show a message in .class-grid
            document.querySelector('.class-grid').innerHTML = `<div class="no-classes-message">Failed to load classes.</div>`;
        }
    });
}
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        loadUserWalletBalance(); // Update wallet balance after cancellation
                    } else {
                        Swal.fire({
                            title: "Canceled!",
...
function showLoader() {
    const loader = document.getElementById('fitpro-loader');
...
// Add this at the end of MyClasses.js
...
// If you use a tab or button to switch to My Classes, add:
document.querySelector('[onclick="myclasses()"]')?.addEventListener('click', function() {
    if (typeof loadUserWalletBalance === "function") {
        loadUserWalletBalance();
    }
});
...
function loadUserWalletBalance() {
    const userId = getUserIdFromToken();
    console.log("Fetching wallet balance for userId:", userId, "from", window.location.pathname);
    ...
    fetch(`http://localhost:8080/api/User/GetUserBalanceById/${userId}`)
        ...
        .then(response => {
            if (response.success && response.data && typeof response.data.balance !== "undefined") {
                ...
                console.log("Wallet balance updated successfully:", response.data.balance);
            } else {
